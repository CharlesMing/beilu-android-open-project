 ...
 * 发现-热门/关注
 */
public class FindInfoListFrag extends BaseMvpFragment<FindInfoView, FindInfoPre<FindInfoView>>
    ...
    public static FindInfoListFrag newInstance(int position) {

        Bundle args = new Bundle();
        args.putInt(FIND_INDEX, position);
        FindInfoListFrag fragment = new FindInfoListFrag();
    ...
    public void initView() {
        super.initView();
        mRecyclerView.addItemDecoration(new FindDivider(mFragmentActivity, DividerItemDecoration.VERTICAL));
    ...
    public void notifyAttention(EventAttentionNotify notify) {
        if (notify.actionAttentionIndex == 0) {
            getPresenter().updateAttention(notify.attentionUserId);
        }
    }
}
